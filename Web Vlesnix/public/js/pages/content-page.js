...
async function saveContent() {
    ...
    try {
        ...
        // Stop editing AFTER the process is finished
        stopEditing(false); 
        ...
        // Clear status message after a few seconds, but only if it's not an error
        if (!statusIndicator.classList.contains('error')) {
            setTimeout(() => {
                if (!isEditing) { // Don't clear if user re-enters edit mode quickly
...
function updateAdminBarUI() {
    if (isEditing) {
        ...
        statusIndicator.textContent = 'Режим редактирования';
        statusIndicator.classList.remove('error', 'success');
    } else {
        editBtn.style.display = 'inline-flex';
        cancelBtn.style.display = 'none';
        saveBtn.style.display = 'none';
        // Status is handled by saveContent, so we don't clear it immediately here
    }
}
