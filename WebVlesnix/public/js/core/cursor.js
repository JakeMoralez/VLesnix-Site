...
function addEventListeners() {
    ...
    const onEnter = (target) => {
        if (target.matches(growSelector)) {
            if (!isStuck) {
                gsap.to(cursor, { duration: 0.3, scale: 1.8, ease: 'power3.out' });
            }
        } else if (target.matches(stickSelector)) {
            isStuck = true;
            stuckTarget = target;
            const rect = stuckTarget.getBoundingClientRect();
            ...
            if (stuckTarget.classList.contains('switch')) {
                ...
                if (slider) br = window.getComputedStyle(slider).borderRadius;
            }

            gsap.to(cursor, {
                duration: 0.3, ease: 'power3.out', width: rect.width, height: rect.height, borderRadius: br,
                backgroundColor: 'transparent', border: `2px solid white`, scale: 1, mixBlendMode: 'normal'
            });
        }
    };
    ...
    const onLeave = (target) => {
        if (target.matches(growSelector)) {
            ...
            stuckTarget = null;
            gsap.to(cursor, {
                duration: 0.3, ease: 'power3.out', width: 20, height: 20, borderRadius: '50%',
                backgroundColor: 'white', border: 'none', mixBlendMode: 'difference', scale: 1
            });
        }
    };

    document.addEventListener('mouseover', (e) => {
        const currentTarget = e.target.closest(`${growSelector}, ${stickSelector}`);
